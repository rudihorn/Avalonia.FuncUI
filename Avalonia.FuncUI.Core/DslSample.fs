...
module rec FuncUI =
    ...
    let MakeAttrT<'a, 'b> (expr : Expr<'a -> 'b>) : 'b -> 'a -> unit = 
        ...
        | Lambda (var, expr) ->
            ...
            | PropertyGet (_, propInfo, _) ->
                ...
                apply
            | _ -> failwith "Not a Property"
        | _ -> failwith "Not a function to a property."

    let MakeAttr'<'a, 'b when 'a :> IControl> (propInfo : Avalonia.DirectProperty<'a, 'b>) : 'b -> 'a -> unit = 
    ...
    let Width = MakeAttrT <@ fun (v : Layoutable) -> v.Width @>
    let Height = MakeAttrT <@ fun (v : Layoutable) -> v.Height @>
    let Background = MakeAttrT <@ fun (v : TemplatedControl) -> v.Background @>

    (* Or define like this, which is typesafe! *)
    let Text = MakeAttr' Avalonia.Controls.TextBlock.TextProperty
